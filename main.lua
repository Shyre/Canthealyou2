...
function CantHealYou_OnEvent(self, event, arg1, arg2, arg3, arg4)
    ...
    elseif event == "SPELL_FAILED_NOT_INCONTROL" or event == "SPELL_FAILED_CHARMED" or event == "SPELL_FAILED_SILENCED" then
      ...
      DEFAULT_CHAT_FRAME:AddMessage(format ("[ %s ] %.2f , %.2f",GetZoneText(),px*100,py*100));
      -- Debug(px)
      /way format ("%.2f , %.2f",px*100,py*100)
...
function CantHealYouOptions_Save()
  ...
  if ((CantHealYouOptionsOutOfRange:GetText() == "") and
      ...
      (CantHealYouOptionsLostControl:GetText() == "") and
      (CantHealYouOptionsGainedControl:GetText() == "") and
      (CantHealYouOptionsAuraBounced:GetText() == "")) then
    return
  end
